...
const std::string file = "input_p7_test.txt";
...
int main(){
    ...
    std::vector<std::vector<int>> timelines; 
    int r = 0;
    int total1 = 0;
    int total2 = 0;
    ...
    for(int i=1; i<obstacles.size();++i){
        ...
        std::vector<int> new_timelines;
        std::cout << i << "/" << obstacles.size()<<std::endl;
        int k = 0;
        for(auto idx : beam_idxs[i-1]){
            if (obs_row[idx] == 1){
                total1++;
                // if ((obs_row[idx-1]==0)&&(std::find(new_beams.begin(), new_beams.end(), idx-1) == new_beams.end())){
                if ((obs_row[idx-1]==0)){
                    ...
                    if (it == new_beams.end()){
                        ...
                        new_timelines[beam_idx_int] += timelines[idx-1][k];
                    }
                    // timelines.push_back
                }
                // if ((obs_row[idx+1]==0)&&(std::find(new_beams.begin(), new_beams.end(), idx+1) == new_beams.end())){
                ...
                if(std::find(new_beams.begin(), new_beams.end(), idx) == new_beams.end()){
